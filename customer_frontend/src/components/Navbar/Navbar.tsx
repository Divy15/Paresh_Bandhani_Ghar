...
export type NavKey = "home" | "contact" | "auth" | "shopping" | "likes" | "bag" | "product-details";
...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab }) => {
  ...
  const renderBadgeIcon = (
    ...
    <button
      type="button"
      onClick={onClick}
      className="relative p-2 rounded-full border border-border-main hover:bg-brand-yellow/10 transition cursor-pointer shrink-0"
      aria-label={label}
    >
      <Icon size={18} className="text-text-main" />
      {count > 0 && (
        <span className="absolute -top-1 -right-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-brand-red px-1 text-[9px] font-bold text-white">
          {count > 99 ? "99+" : count}
        </span>
      )}
    </button>
        ...
        <div className="flex items-center justify-end space-x-4 font-body text-sm">
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-3">
              {renderBadgeIcon(Heart, likeCount, "View wishlist", () => {
                setActiveTab("likes");
                setIsProfileMenuOpen(false);
              })}
              {renderBadgeIcon(ShoppingBag, bagCount, "View shopping bag", () => {
                setActiveTab("bag");
                setIsProfileMenuOpen(false);
              })}
